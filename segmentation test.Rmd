...
for (Segmentation_number in 1:totalseg){
  ...
  Protein_Summary_bind_Score_max<-Protein_Summary_bind_Score %>% group_by(.dots=c("Segmentation_number")) %>% summarise(Peptide_Score=(mean(Peptide_Score)))
...
library(FTICRMS)
  ...
  common_feature<-common_feature[common_feature$count>=0.2*totalseg,]
  ...
  g<-ggplot(locate.peaks_spec_bindlist_bind_filtered)+geom_bar(aes(x=Segmentation_number, y=Width_hat, position = "dodge", , fun.y = "median"))
  g
  ...
  ggplot(NULL,aes(x=peak_num,y=peak_width)) +
  geom_bar(stat="identity")
...
for (indx in 1:length(enzyme)){
  ...
  #Protein_Summary_bind_protein<-Protein_Summary_bind %>% group_by(indx) %>% summarise(Protein=(unique(Protein)))
  ...
  ggplot2::ggplot(Protein_Summary_bind_proteinnr, aes(x=indx,group=indx)) + geom_bar()
  
  
 
  plot(Protein_Summary_bind_peptide)
  plot(Protein_Summary_bind_Score_max)
  Protein_Summary_bind_peptide_id_quality<-data.frame(indx=Protein_Summary_bind_peptide$indx,ID_score_count=Protein_Summary_bind_peptide$Peptide_num*Protein_Summary_bind_Score_max$Peptide_Score)
  plot(Protein_Summary_bind_peptide_id_quality)
