...
#'   \item peakPick: "diff", "sd", "mad", "quantile", "filter", "cwt"
...
validate_preprocess_params <- function(preprocess = list(), ppm = 5, memory_load = TRUE) {
  ...
  valid_methods <- list(
    ...
    reduceBaseline = c("Disable", "locmin", "hull", "snip", "median"),
    peakPick = c("Disable", "Default", .hitmap_cardinal_peak_pick_methods()),
    normalize = c("Disable", "tic", "rms", "reference")
  )
  ...
  complete_preprocess <- .hitmap_normalize_preprocess_methods(complete_preprocess)
  ...
  if (is.null(complete_preprocess$peakPick$method)) {
    complete_preprocess$peakPick$method <- "mad"
  } else if (!complete_preprocess$peakPick$method %in% valid_methods$peakPick) {
    warning("Invalid peakPick method. Using 'mad'. Valid options: ",
...
load_cardinal_data <- function(datafile, memory = TRUE, resolution = 2.5,
  ...
  if (!grepl("\\.imzML$", datafile, ignore.case = TRUE)) {
    datafile <- paste0(datafile, ".imzML")
  }
